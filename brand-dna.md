...
| 色名 | 色值 | 用途 |
|------|------|------|
| 薄荷绿 | `#BDE7E5` | 主色调、标题、超链接、重点标记 |
| 淡粉 | `#F4CFD6` | 强调、装饰、badges、连接线、高亮 |
| 深棕 | `#694B40` | 点缀、高亮下划线、CTA、标签、文字主色 |

三色比例原则：主色 60% · 强调色 30% · 点缀色 10%（点缀色永远是点缀，不做主色）

> 💡 如需替换为你自己的品牌色，改上面三个色值即可，比例原则不变。不知道怎么选色？参考 `推荐色板.md`（如有）。
...
- 背景永远偏暖：`#FBF2E3`（主背景）、`#F5E6D3`（深奶）
- 文字永远非纯黑：用 `#694B40`（暖棕墨色）或 `#8B6F5C`
- 次要文字：`#8B6F5C`、`#A89488`
- 绝不用纯黑 `#000` 或纯白 `#fff`
- 暗色场景底色：`#5A4037`、`#4A352E`（暖棕调暗底，仅适用于 HTML 全屏页面，3:4 卡片场景禁止深色底）
- 薄荷绿浅色：`#E8F6F5`（用于 hover、轻量背景）
- 淡粉浅色：`#FCE8EC`（用于引用框、高亮底色）
- 径向渐变制造层次，不用纯平色
